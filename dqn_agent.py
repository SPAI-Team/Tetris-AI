...
class DQNAgent:
    ...
    def train(self, batch_size=32, epochs=3, episode = 1):
        '''Trains the agent'''
        n = len(self.memory)
        ...
        if n >= self.replay_start_size and n >= batch_size:
            ...
            checkpoint_path = f"training_2/cp-{episode}.ckpt"
            ...
            self.model.fit(np.array(x), np.array(y), batch_size=batch_size, epochs=epochs, verbose=0, callbacks = [cp_callback])
